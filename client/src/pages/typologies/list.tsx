...
import TypologyForm from "@/components/typologies/typology-form";

export default function TypologiesList() {
  const { isAuthenticated, isLoading } = useAuth();
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
  const [formKey, setFormKey] = useState(0);
  const [editItem, setEditItem] = useState<Typology | null>(null);
  const [confirmOpen, setConfirmOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<Typology | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (t: Typology) => deleteRequest(t.id),
    onMutate: async (t) => {
      await queryClient.cancelQueries({ queryKey: ["/api/typologies"] });
      const prev = queryClient.getQueryData<Typology[]>(["/api/typologies"]) || [];
      queryClient.setQueryData<Typology[]>(["/api/typologies"], prev.filter(x => x.id !== t.id));
      return { prev };
    },
    onError: (err, _t, ctx) => {
                    ...
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <ActiveToggleButton id={t.id} resource="typologies" isActive={(t as any).isActive} queryKey={["/api/typologies"]} entityLabel="Tipo" />
      ...
      <Dialog open={open} onOpenChange={(v) => { if (v) setFormKey(k => k + 1); if (!v) setEditItem(null); setOpen(v); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] p-0 overflow-hidden">
          <div className="max-h-[calc(90vh-1rem)] overflow-y-auto my-7 px-7">
            <TypologyForm key={formKey} initialItem={editItem} onSuccess={() => { queryClient.invalidateQueries({ queryKey: ["/api/typologies"] }); if (editItem) { setEditItem(null); setOpen(false); } }} onCancel={() => setOpen(false)} />
          </div>
        </DialogContent>
      </Dialog>

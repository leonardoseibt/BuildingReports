...
export default function RequirementForm({ initialItem, onSuccess, onCancel }: { initialItem: Requirement | null; onSuccess?: () => void; onCancel?: () => void; }) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      const url = initialItem ? `/api/requirements/${initialItem.id}` : '/api/requirements';
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-6" autoComplete="off">
        <FormHeader title={initialItem ? 'Editar Requisito' : 'Novo Requisito'} subtitle={initialItem ? 'Atualize os dados do requisito.' : 'Cadastre um novo requisito.'} initials={initialItem?.code ?? null} />
            ...
            <FormItem className="md:col-span-1">
              <FormControl>
                <NotchedField label="Código" requiredMark>
                  <Input placeholder="1" {...field} className="bg-transparent border-0 shadow-none focus-visible:ring-0 focus-visible:ring-offset-0" />
                </NotchedField>
              </FormControl>
              <FormMessage />
            </FormItem>
            ...
            <FormItem className="md:col-span-2">
              <FormControl>
                <NotchedField label="Descrição" requiredMark>
                  <Input placeholder="Gerais" {...field} className="bg-transparent border-0 shadow-none focus-visible:ring-0 focus-visible:ring-offset-0" />
                </NotchedField>
              </FormControl>
              <FormMessage />
            </FormItem>

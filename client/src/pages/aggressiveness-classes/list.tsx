...
export default function AggressivenessClassesList() {
  ...
  const pagedItems = useMemo(() => sorted.slice((pageSafe - 1) * pageSize, pageSafe * pageSize), [sorted, pageSafe, pageSize]);
          ...
          {isLoadingItems ? (
            ...
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60">
              <Table className="table-fixed">
                ...
                <TableBody>
                  {pagedItems.map((t: AggressivenessClass) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <ActiveToggleButton id={t.id} resource="aggressiveness-classes" isActive={(t as any).isActive} queryKey={["/api/aggressiveness-classes"]} entityLabel="Classe" />
      ...
      <Dialog open={open} onOpenChange={(v) => { if (v) setFormKey(k => k + 1); if (!v) setEditItem(null); setOpen(v); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] p-0 overflow-hidden">
          <div className="max-h-[calc(90vh-1rem)] overflow-y-auto my-7 px-7">
            <AggressivenessClassForm key={formKey} initialItem={editItem} onSuccess={() => { queryClient.invalidateQueries({ queryKey: ["/api/aggressiveness-classes"] }); if (editItem) { setEditItem(null); setOpen(false); } }} onCancel={() => setOpen(false)} />
          </div>
        </DialogContent>
      </Dialog>

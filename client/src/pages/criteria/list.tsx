...
export default function CriteriaList() {
  ...
  const [selectedItem, setSelectedItem] = useState<Criterion | null>(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (t: Criterion) => deleteRequest(t.id),
    onMutate: async (t) => {
      await queryClient.cancelQueries({ queryKey: ["/api/criteria"] });
      const prev = queryClient.getQueryData<Criterion[]>(["/api/criteria"]) || [];
      queryClient.setQueryData<Criterion[]>(["/api/criteria"], prev.filter(x => x.id !== t.id));
      return { prev };
    },
    onError: (err, _t, ctx) => {
      if (ctx?.prev) queryClient.setQueryData(["/api/criteria"], ctx.prev);
      toast({ title: 'Erro ao excluir', description: String(err), variant: 'destructive' });
    },
    onSuccess: (_data, t) => { toast({ title: 'Critério excluído', description: `${t.label} foi removido.` }); },
    onSettled: () => { queryClient.invalidateQueries({ queryKey: ["/api/criteria"], refetchType: 'inactive' }); }
  });

  function askDelete(t: Criterion) { setSelectedItem(t); setConfirmOpen(true); }
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header
          title="Critérios"
          description="Gerencie as categorias de critérios de desempenho"
          action={
        ...
        <main className="flex-1 overflow-y-auto p-6">
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-full max-w-lg">
                <input
                  ...
                  placeholder="Buscar critérios (código, descrição, data)"
          ...
          {isLoadingItems ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-slate-900 mb-2">Nenhum critério cadastrado</h3>
              ...
              <Button size="lg" onClick={() => { setEditItem(null); setFormKey(k => k + 1); setOpen(true); }}>
                <Plus className="w-4 h-4 mr-2" /> Cadastrar Critério
              </Button>
            </div>
          ) : (
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60">
                    ...
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <ActiveToggleButton id={t.id} resource="criteria" isActive={(t as any).isActive} queryKey={["/api/criteria"]} entityLabel="Critério" />
              ...
              <div className="flex items-center justify-between gap-4 border-t px-4 py-3 text-sm text-slate-600">
                <p>
                  ...
                  <span className="font-semibold">{filtered.length}</span> critérios
                </p>
      ...
      <Dialog open={open} onOpenChange={(v) => { if (v) setFormKey(k => k + 1); if (!v) setEditItem(null); setOpen(v); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] p-0 overflow-hidden">
          <div className="max-h-[calc(90vh-1rem)] overflow-y-auto my-7 px-7">
            <CriterionForm key={formKey} initialItem={editItem} onSuccess={() => { queryClient.invalidateQueries({ queryKey: ["/api/criteria"] }); if (editItem) { setEditItem(null); setOpen(false); } }} onCancel={() => setOpen(false)} />
          </div>
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir {selectedItem ? (<strong>{` ${selectedItem.label} `}</strong>) : ("este critério")}? 
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
export default function ParametersList() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(t => {
      const analysisLabel = analysisLabelById.get(t.analysisId) || '';
      return normText(t.label).includes(q) || 
             normText((t as any).unit).includes(q) ||
             normText(analysisLabel).includes(q);
    });
  }, [items, search, analysisLabelById]);
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-col md:flex-row gap-3 flex-1">
            ...
            <select
              ...
              className="h-9 text-sm rounded-md border px-2 bg-white focus:outline-none"
            >
  ...
  {isLoadingItems ? (<div className="text-center py-12"><ListChecks className="w-16 h-16 text-slate-300 mx-auto mb-4"/><p className="text-slate-500">Carregando...</p></div>) : items.length===0 ? (<div className="text-center py-12"><ListChecks className="w-16 h-16 text-slate-300 mx-auto mb-4"/><h3 className="text-lg font-medium text-slate-900 mb-2">Nenhum parâmetro cadastrado</h3><p className="text-slate-500 mb-6">Cadastre o primeiro.</p><Button size="lg" onClick={()=>{ setEditItem(null); setFormKey(k=>k+1); setOpen(true); }}><Plus className="w-4 h-4 mr-2"/> Cadastrar Parâmetro</Button></div>) : pagedItems.length===0 ? (<div className="text-center py-12"><ListChecks className="w-16 h-16 text-slate-300 mx-auto mb-4"/><h3 className="text-lg font-medium text-slate-900 mb-2">Nenhum resultado encontrado</h3><p className="text-slate-500">Tente ajustar os filtros de busca.</p></div>) : (<div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60"><Table className="table-fixed"><TableHeader><TableRow className="bg-slate-100/60"><TableHead onClick={()=>toggleSort('analysisId')} aria-sort={sortBy==='analysisId'?(sortDir==='asc'?'ascending':'descending'):'none'} className="w-[25%] cursor-pointer select-none">Análise {sortBy==='analysisId' && (sortDir==='asc'? <ArrowUp className="inline-block w-3 h-3 ml-1 opacity-70"/>:<ArrowDown className="inline-block w-3 h-3 ml-1 opacity-70"/> )}</TableHead><TableHead onClick={()=>toggleSort('label')} aria-sort={sortBy==='label'?(sortDir==='asc'?'ascending':'descending'):'none'} className="w-[30%] cursor-pointer select-none">Descrição {sortBy==='label' && (sortDir==='asc'? <ArrowUp className="inline-block w-3 h-3 ml-1 opacity-70"/>:<ArrowDown className="inline-block w-3 h-3 ml-1 opacity-70"/> )}</TableHead><TableHead className="w-[8%]">Min</TableHead><TableHead className="w-[8%]">Int</TableHead><TableHead className="w-[8%]">Sup</TableHead><TableHead onClick={()=>toggleSort('unit')} aria-sort={sortBy==='unit'?(sortDir==='asc'?'ascending':'descending'):'none'} className="w-[7%] cursor-pointer select-none">Unidade {sortBy==='unit' && (sortDir==='asc'? <ArrowUp className="inline-block w-3 h-3 ml-1 opacity-70"/>:<ArrowDown className="inline-block w-3 h-3 ml-1 opacity-70"/> )}</TableHead><TableHead onClick={()=>toggleSort('isActive')} aria-sort={sortBy==='isActive'?(sortDir==='asc'?'ascending':'descending'):'none'} className="w-[6%] cursor-pointer select-none">Ativo {sortBy==='isActive' && (sortDir==='asc'? <ArrowUp className="inline-block w-3 h-3 ml-1 opacity-70"/>:<ArrowDown className="inline-block w-3 h-3 ml-1 opacity-70"/> )}</TableHead><TableHead className="w-[16%] text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{pagedItems.map(t=>{ const analysis = analyses.find(a=>a.id===t.analysisId); return (<TableRow key={t.id}><TableCell className="font-medium break-words">{analysis?analysis.label:t.analysisId}</TableCell><TableCell><pre className="whitespace-pre-line break-words font-sans text-sm m-0 p-0 bg-transparent border-0 shadow-none">{t.label}</pre></TableCell><TableCell>{(t as any).minimumValue ?? '-'}</TableCell><TableCell>{(t as any).intermediateValue ?? '-'}</TableCell><TableCell>{(t as any).superiorValue ?? '-'}</TableCell><TableCell>{(t as any).unit ?? '-'}</TableCell><TableCell>{t.isActive?'Sim':'Não'}</TableCell><TableCell className="text-right"><div className="flex items-center justify-end gap-1.5"><Button variant="ghost" size="icon" onClick={()=>{ setEditItem(t); setFormKey(k=>k+1); setOpen(true); }}><Pencil className="h-4 w-4"/></Button><ActiveToggleButton id={t.id} resource="parameters" isActive={t.isActive as any} queryKey={parametersKey} entityLabel="Parâmetro"/><Button variant="ghost" size="icon" className="text-rose-600 hover:bg-rose-50 hover:text-rose-700" onClick={()=>askDelete(t)} disabled={deleteMutation.isPending && selectedItem?.id===t.id}><Trash2 className="h-4 w-4"/></Button></div></TableCell></TableRow>); })}</TableBody></Table><div className="flex items-center justify-between gap-4 border-t px-4 py-3 text-sm text-slate-600"><p>Mostrando <span className="font-semibold">{pagedItems.length}</span> de <span className="font-semibold">{filtered.length}</span> parâmetros</p><Pagination totalPages={totalPages} page={pageSafe} onPageChange={(p:number)=> setPage(p)} /></div></div>) }
    </main></div>
    <Dialog open={open} onOpenChange={(v)=>{ if(v) setFormKey(k=>k+1); if(!v) setEditItem(null); setOpen(v); }}>
  <DialogContent className="max-w-4xl max-h-[90vh] p-0 overflow-hidden"><div className="max-h-[calc(90vh-1rem)] overflow-y-auto my-7 px-7"><ParameterForm key={formKey} initialItem={editItem} onSuccess={()=>{ queryClient.invalidateQueries({ predicate: q => Array.isArray(q.queryKey) && q.queryKey[0] === '/api/parameters' }); if (editItem) { setEditItem(null); setOpen(false); } }} onCancel={()=> setOpen(false)} /></div></DialogContent>
    </Dialog>

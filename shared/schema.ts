...
export const userSettings = pgTable(
  "user_settings",
  {
    ...
    pageSize: integer("page_size").notNull().default(15),
    createdAt: timestamp("created_at").defaultNow(),
    updatedAt: timestamp("updated_at").defaultNow(),
  },
  (table) => [uniqueIndex("uq_user_settings_user").on(table.userId)]
);
...
export const noiseClasses = pgTable("noise_classes", {
  ...
  code: varchar("code", { length: 64 }).notNull().unique(),
  label: varchar("label", { length: 255 }).notNull(),
  minDb: integer('min_db').notNull().default(0),
  maxDb: integer('max_db'),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

...
async function main() {
  console.log('\n=== Verificando Configuração do Atributo ===\n');
  ...
  const params = await db.execute(sql`
    SELECT p.id, p.label, p.attribute_id, p.attribute_value_id
    FROM parameters p
  ...
  params.rows.forEach((row: any) => {
    console.log(`\nParâmetro ${row.id}: ${row.label.substring(0, 60)}...`);

...
export default function RequirementsList() {
  ...
  // Redireciona se sessão expirar
  useAuthRedirect();

  const { data: items = [], isFetching, isLoading: isLoadingItems } = useQuery<Requirement[]>({ queryKey: ["/api/requirements"], enabled: isAuthenticated });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (t: Requirement) => deleteRequest(t.id),
    onMutate: async (t) => {
      await queryClient.cancelQueries({ queryKey: ["/api/requirements"] });
      const prev = queryClient.getQueryData<Requirement[]>(["/api/requirements"]) || [];
      queryClient.setQueryData<Requirement[]>(["/api/requirements"], prev.filter(x => x.id !== t.id));
      return { prev };
    },
    onError: (err, _t, ctx) => {
      if (ctx?.prev) queryClient.setQueryData(["/api/requirements"], ctx.prev);
      toast({ title: 'Erro ao excluir', description: String(err), variant: 'destructive' });
    },
    onSuccess: (_data, t) => { toast({ title: 'Requisito excluído', description: `${t.label} foi removido.` }); },
    onSettled: () => { queryClient.invalidateQueries({ queryKey: ["/api/requirements"], refetchType: 'inactive' }); }
  });

  function askDelete(t: Requirement) { setSelectedItem(t); setConfirmOpen(true); }
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header
          title="Requisitos"
          description="Gerencie as categorias de requisitos"
          action={
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => { setEditItem(null); setFormKey(k => k + 1); setOpen(true); }}>
                <Plus className="w-4 h-4 mr-2" /> Novo Requisito
              </Button>
            </div>
          }
        />
        <main className="flex-1 overflow-y-auto p-6">
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-full max-w-lg">
                <input
                  ...
                  placeholder="Buscar requisitos (código, descrição, data)"
          ...
          {isLoadingItems ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-slate-900 mb-2">Nenhum requisito cadastrado</h3>
              ...
              <Button size="lg" onClick={() => { setEditItem(null); setFormKey(k => k + 1); setOpen(true); }}>
                <Plus className="w-4 h-4 mr-2" /> Cadastrar Requisito
              </Button>
            </div>
          ) : (
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60">
              ...
              <div className="flex items-center justify-between gap-4 border-t px-4 py-3 text-sm text-slate-600">
                <p>
                  ...
                  <span className="font-semibold">{filtered.length}</span> requisitos
                </p>
      ...
      <Dialog open={open} onOpenChange={(v) => { if (v) setFormKey(k => k + 1); if (!v) setEditItem(null); setOpen(v); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] p-0 overflow-hidden">
          <div className="max-h-[calc(90vh-1rem)] overflow-y-auto my-7 px-7">
            <RequirementForm key={formKey} initialItem={editItem} onSuccess={() => { queryClient.invalidateQueries({ queryKey: ["/api/requirements"] }); if (editItem) { setEditItem(null); setOpen(false); } }} onCancel={() => setOpen(false)} />
          </div>
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir {selectedItem ? (<strong>{` ${selectedItem.label} `}</strong>) : ("este requisito")}? 
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

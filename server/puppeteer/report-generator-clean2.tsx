...
const levelOrder: Array<"minimum" | "intermediate" | "superior"> = [
  "minimum",
  "intermediate",
  "superior",
];
...
const technicalFieldMap = technicalFields.reduce<Record<string, string>>(
  (acc, field) => {
...
function hasValuesForSelectedLevels(
  parameter: Parameter,
...
function getAttributeValue(
  ...
  if (
    sourceData[attribute.sourceColumn] !== undefined &&
    sourceData[attribute.sourceColumn] !== null
  ) {
  ...
  if (attribute.sourceTable === "buildings") {
    ...
    if (
      camel &&
      sourceData[camel] !== undefined &&
      sourceData[camel] !== null
    ) {
...
function findRelatedRecord(
  ...
  const strategies: Array<() => any> = [
    () => {
      ...
      const buildingValue =
        (building as any)[camelCaseId] || (building as any)[snakeCaseId];
...
function shouldShowParameter(
  ...
  if (
    parameter.attributeValueId !== null &&
    parameter.attributeValueId !== undefined
  ) {
    if (String(parameter.attributeValueId) !== String(attributeValue))
      return false;
  }
  ...
  return [...params].sort((a, b) => {
    ...
    const project = (param: Parameter) => {
      ...
      if (bestNumeric === Number.MAX_SAFE_INTEGER) {
        ...
        return {
          numericValue: Number.MAX_SAFE_INTEGER,
          columnPriority: entries[0].priority,
          textValue,
        };
      }

      return {
        numericValue: bestNumeric,
        columnPriority: bestPriority,
        textValue: undefined,
      };
    };
    ...
    if (aData.numericValue !== bData.numericValue)
      return aData.numericValue - bData.numericValue;

    if (aData.columnPriority !== bData.columnPriority)
      return aData.columnPriority - bData.columnPriority;

    if (aData.textValue && bData.textValue)
      return aData.textValue.localeCompare(bData.textValue, "pt-BR");
...
function normalizeDisplayValue(value: unknown): string {
  ...
  if (lowered === "false" || lowered === "null" || lowered === "undefined")
    return "\u2014";
...
function buildBuildingInfoSections(
  ...
  const displayName =
    buildingName || (fallbackId ? `Edificacao ID ${fallbackId}` : "Edificacao");
...
function getAggressivenessClassInfo(
  building: Building,
  aggressiveness: AggressivenessClass[],
): string | null {
  if (!building.aggressivenessClassId) return null;

  const item = aggressiveness.find(
    (a) => a.id === building.aggressivenessClassId,
  );
...
function getTechnicianInfo(
  ...
  return item
    ? `${item.fullName} (${item.creaCau ?? "CREA/CAU"})`
    : `ID ${building.technicianId}`;
}

function getBioclimaticZoneInfo(
  building: Building,
  zones: BioclimaticZone[],
): string | null {
...
function buildFilename(building: Building, report: Report): string {
  const name = building.name
    ? building.name.replace(/[^a-zA-Z0-9-_]+/g, "_")
    : "Relatorio";
...
function ReportHtml({ context }: { context: ReportRenderContext }) {
  ...
  const buildingInfoSections = buildBuildingInfoSections(
    building,
    context.report,
    {
      typologies,
          ...
          {buildingInfoSections.length > 0 && (
            <div className="section" style={{ marginBottom: "0" }}>
              {buildingInfoSections.map((section: BuildingInfoSection) => {
                const showUnitColumn = section.rows.some(
                  (row: BuildingInfoRow) => {
                ...
                return (
                  <div key={section.title} className="building-info-section">
                    ...
                    <table className="building-info-table">
                      <tbody>
                        {section.rows.map(
                          (row: BuildingInfoRow, index: number) => (
                            <tr key={`${section.title}-${row.label}-${index}`}>
                              <td className="building-info-label">
                                {formatWithSeparators(row.label)}
                              </td>
                              ...
                              {showUnitColumn && (
                                <td className="building-info-unit">
                                  {row.unit
                                    ? formatWithSeparators(row.unit)
                                    : "-"}
                                </td>
                              )}
                            </tr>
        ...
        {sections.map((requirement) => (
          <div key={requirement.id} className="section">
            <h2 className="section-title">
              Requisito: {normalizeText(requirement.label)}
            </h2>
                      ...
                      <table>
                        <thead>
                          ...
                          <tr>
                            {columns.map((column, index) => (
                              <th
                                key={`${analysis.id}-${column}-${index}`}
                                className={
                                  index === 0 ? "param-col" : "value-col"
                                }
                              >
                                {column}
                              </th>
                        ...
                        <tbody>
                          {analysis.parameters.map((parameter) => {
                            const observation =
                              parameter.notes ?? (parameter as any).observation;
                            return (
                              <tr key={parameter.id}>
                                <td className="param-col">
                                  <div className="param-label">
                                    {formatWithSeparators(parameter.label)}
                                  </div>
                                  {observation && (
                                    <div className="param-observation">
                                      {formatWithSeparators(observation)}
                                    </div>
                                  )}
                                </td>
                                ...
                                {analysis.selectedLevels.map((level) => {
                                  ...
                                  const nested = (parameter as any).values?.[
                                    level
                                  ];
                                  ...
                                  return (
                                    <td key={level} className="value-col">
...
async function loadTableData(
  ...
  switch (table) {
    ...
    case "technicians": {
      const { items } = await storage.listTechnicians(
        userId,
        undefined,
        undefined,
      );
...
async function loadReportContext(
  ...
  const evaluations = Array.isArray((reportData as any).evaluations)
    ? (reportData as any).evaluations
    : [];
  ...
  const groupedData = requirements
    ...
    .map((requirement) => {
      const criteriaForRequirement = criteria
        ...
        .filter((criterion) =>
          analysesRaw.some(
            (analysis) =>
              analysis.requirementId === requirement.id &&
              analysis.criterionId === criterion.id,
          ),
        )
          ...
          const analysesForCriterion = analysesRaw
            ...
            .map((analysis) => {
              const params = parametersRaw
                ...
                .filter(
                  (parameter) =>
                    parameter.analysisId === analysis.id &&
            ...
            .filter((analysis) => Boolean(analysis)) as (Analysis & {
            parameters: Parameter[];
          })[];
        ...
        .filter((criterion) => Boolean(criterion)) as (Criterion & {
        analyses: (Analysis & { parameters: Parameter[] })[];
      })[];
    ...
    .filter((requirement) => Boolean(requirement)) as (Requirement & {
    criteria: (Criterion & {
      analyses: (Analysis & { parameters: Parameter[] })[];
    })[];
  })[];
      ...
      const mappedCriteria = requirement.criteria
        ...
        .map((criterion) => {
          const analysesWithLevels = criterion.analyses
            ...
            .map((analysis) => {
              let selectedLevels =
                selectedEvaluations.get(`analysis-${analysis.id}`)?.slice() ??
                levelOrder.slice();

              const filteredParameters = analysis.parameters.filter(
                (parameter) =>
    ...
    .map((requirement) => ({
      ...
      criteria: requirement.criteria
        ...
        .map((criterion) => ({
          ...
          analyses: criterion.analyses.sort((a, b) =>
            a.code.localeCompare(b.code, "pt-BR", {
              numeric: true,
              sensitivity: "base",
            }),
...
export async function generateReportPdf(
  ...
  const html =
    "<!DOCTYPE html>" + renderToStaticMarkup(<ReportHtml context={context} />);
